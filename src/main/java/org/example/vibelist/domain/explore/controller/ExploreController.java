...
public class ExploreController {
    ...
    @ApiResponse(responseCode = "200", description = "검색된 게시글 목록을 반환합니다.")
    @GetMapping("/search")
    public Page<PostDetailResponse> search(
            @Parameter(description = "검색 키워드", required = true)
            @RequestParam("q") String keyword,

            @Parameter(hidden = true)
            Pageable pageable) {
        return exploreService.search(keyword, pageable);
    }

    @Operation(summary = "피드 조회", description = "추천 게시글 피드를 조회합니다.")
    @ApiResponse(responseCode = "200", description = "추천 피드 게시글 목록을 반환합니다.")
    @GetMapping("/feed")
    public Page<PostDetailResponse> feed(
            @Parameter(hidden = true)
            Pageable pageable) {
        return exploreService.feed(pageable);
    }

    @Operation(summary = "트렌드 조회", description = "현재 인기 있는 트렌드를 조회합니다.")
    @ApiResponse(responseCode = "200", description = "트렌드 키워드 목록을 반환합니다.")
    @GetMapping("/trend")
